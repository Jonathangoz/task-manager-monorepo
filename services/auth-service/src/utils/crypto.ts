...
export class CryptoUtils {
  ...
  static async verifyPassword(hash: string, password: string): Promise<boolean> {
    ...
    try {
      // Validar entradas
      if (!hash || !password || typeof hash !== 'string' || typeof password !== 'string') {
  ...
  static validatePasswordStrength(password: string): PasswordStrengthResult {
    ...
    if (password.length > PASSWORD_VALIDATION.MAX_LENGTH) {
      errors.push(`La contraseña debe tener menos de ${PASSWORD_VALIDATION.MAX_LENGTH} caracteres`);
    }
    ...
    // Validar símbolos
    if (PASSWORD_VALIDATION.REQUIRE_SYMBOLS && !/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~`]/.test(password)) {
  ...
  static verifyCryptoSetup(): {
    ...
    if (environment.app.isProduction) {
      if (this.config.memoryCost < 2 ** 15) { // Menos de 32MB
