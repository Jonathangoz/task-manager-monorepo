...
import {
  ...
  CacheOptions,
  RateLimitResult,
} from '@/core/interfaces/ICacheService';
...
export class RedisCache implements ICacheService {
  ...
  async set<T>(key: string, value: T, options?: CacheOptions): Promise<void> {
    try {
      ...
      if (validatedOptions?.ttl) {
        ...
        if (validatedOptions.nx) {
          const result = await this.client.set(
            ...
            serializedValue,
            'EX',
            validatedOptions.ttl,
            'NX',
          );
          if (!result) {
            redisLogger.debug(
              { key: validatedKey },
              'Key already exists, not set due to NX option',
            );
            return;
          }
        } else {
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid cache parameters`,
        );
      }
  ...
  async del(key: string): Promise<number> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, deleted: result > 0 },
        'Key deletion result',
      );
      return result;
    } catch (error) {
  ...
  async exists(key: string): Promise<boolean> {
    try {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to check key existence`,
      );
    }
  }
  ...
  async expire(key: string, seconds: number): Promise<boolean> {
    try {
      ...
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { key, seconds, error: error.errors },
          'Invalid expire parameters',
        );
        return false;
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to set key expiration`,
      );
    }
  }
  ...
  async sadd(key: string, members: string[]): Promise<number> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, membersCount: members.length, added: result },
        'Members added to set',
      );
      return result;
    } catch (error) {
      logError.medium(error as Error, {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to add members to set`,
      );
    }
  }
  ...
  async srem(key: string, members: string[]): Promise<number> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, membersCount: members.length, removed: result },
        'Members removed from set',
      );
      return result;
    } catch (error) {
      logError.medium(error as Error, {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to remove members from set`,
      );
    }
  }
  ...
  async smembers(key: string): Promise<string[]> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, membersCount: members.length },
        'Set members retrieved',
      );
      return members;
    } catch (error) {
      logError.medium(error as Error, {
  ...
  async sismember(key: string, member: string): Promise<boolean> {
    try {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to check set membership`,
      );
    }
  }
  ...
  async hget(key: string, field: string): Promise<string | null> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, field, found: !!value },
        'Hash field retrieved',
      );
      return value;
    } catch (error) {
      logError.medium(error as Error, {
  ...
  async hdel(key: string, field: string): Promise<number> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, field, deleted: result > 0 },
        'Hash field deletion result',
      );
      return result;
    } catch (error) {
      logError.medium(error as Error, {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to delete hash field`,
      );
    }
  }
  ...
  async hgetall(key: string): Promise<Record<string, string>> {
    try {
      ...
      redisLogger.debug(
        { key: validatedKey, fieldsCount: Object.keys(result).length },
        'Hash retrieved',
      );
      return result;
    } catch (error) {
  ...
  async storeSession(
    ...
    try {
      ...
      redisLogger.info(
        { sessionId: validatedSessionId, ttl: validatedTTL },
        'Session stored successfully',
      );
    } catch (error) {
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { sessionId, error: error.errors },
          'Invalid session parameters',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid session parameters`,
        );
      }
  ...
  async getSession<T>(sessionId: string): Promise<T | null> {
    try {
      ...
      redisLogger.debug(
        { sessionId: validatedSessionId, found: !!session },
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      redisLogger.info(
        { sessionId: validatedSessionId, deleted },
  ...
  async storeRefreshToken(
    ...
    try {
      ...
      redisLogger.info(
        { tokenId: validatedTokenId, ttl: validatedTTL },
        'Refresh token stored successfully',
      );
    } catch (error) {
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { tokenId, error: error.errors },
          'Invalid token parameters',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid token parameters`,
        );
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to store refresh token`,
      );
    }
  }
  ...
  async getRefreshToken<T>(tokenId: string): Promise<T | null> {
    try {
      ...
      redisLogger.debug(
        { tokenId: validatedTokenId, found: !!token },
        'Refresh token retrieval result',
      );
      return token;
    } catch (error) {
      if (error instanceof z.ZodError) {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to get refresh token`,
      );
    }
  }
  ...
  async deleteRefreshToken(tokenId: string): Promise<boolean> {
    try {
      ...
      redisLogger.info(
        { tokenId: validatedTokenId, deleted },
        'Refresh token deletion result',
      );
      return deleted;
    } catch (error) {
      if (error instanceof z.ZodError) {
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to delete refresh token`,
      );
    }
  }
  ...
  async incrementRateLimit(
    ...
    try {
      ...
      const resetTime =
        Date.now() +
        (currentTTL > 0 ? currentTTL * 1000 : validatedWindow * 1000);
      ...
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { key, windowSeconds, error: error.errors },
          'Invalid rate limit parameters',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid rate limit parameters`,
        );
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to increment rate limit`,
      );
    }
  }
  ...
  async recordLoginAttempt(email: string): Promise<number> {
    try {
      ...
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { email, error: error.errors },
          'Invalid email format for login attempt',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid email format`,
        );
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to record login attempt`,
      );
    }
  }
  ...
  async clearLoginAttempts(email: string): Promise<boolean> {
    try {
      ...
      redisLogger.info(
        { email: validatedEmail, cleared },
        'Login attempts clear result',
      );
      return cleared;
    } catch (error) {
      if (error instanceof z.ZodError) {
  ...
  private validateConnection(): void {
    if (!this.isConnected) {
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Redis client is not connected`,
      );
    }
  }
  ...
  async deleteByPattern(pattern: string): Promise<number> {
    try {
      ...
      redisLogger.warn(
        { pattern: validatedPattern, deletedCount },
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to delete keys by pattern`,
      );
    }
  }
  ...
  async setUserSessions(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { userId, error: error.errors },
          'Invalid user sessions parameters',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid user sessions parameters`,
        );
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to set user sessions`,
      );
    }
  }
  ...
  async setUserProfile(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        redisLogger.warn(
          { userId, error: error.errors },
          'Invalid user profile parameters',
        );
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid user profile parameters`,
        );
      }
      ...
      throw new Error(
        `${ERROR_CODES.REDIS_ERROR}: Failed to cache user profile`,
      );
    }
  }
  ...
  async deleteUserData(userId: string): Promise<void> {
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new Error(
          `${ERROR_CODES.VALIDATION_ERROR}: Invalid user ID format`,
        );
      }
  ...
  async flushAll(): Promise<void> {
    try {
      ...
      redisLogger.warn(
        'All cache data flushed - this should only be done in development/testing',
      );
    } catch (error) {
      logError.critical(error as Error, {
        operation: 'flushAll',
