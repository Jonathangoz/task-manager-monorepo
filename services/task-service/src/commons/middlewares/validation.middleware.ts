...
export class ValidationMiddleware {
   ...
   */
  static readonly cuidSchema = z
    .string()
    .min(
      VALIDATION_CONFIG.CUID.MIN_LENGTH,
      'CUID debe tener al menos 20 caracteres',
    )
    .max(
      VALIDATION_CONFIG.CUID.MAX_LENGTH,
      'CUID debe tener máximo 30 caracteres',
    )
   ...
   */
  static readonly hexColorSchema = z
    .string()
    .regex(
      VALIDATION_CONFIG.HEX_COLOR.PATTERN,
      'Color debe estar en formato hexadecimal (#RGB o #RRGGBB)',
    )
    .refine(
   ...
   */
  static readonly iconSchema = z
    .string()
    .min(1, 'Icono no puede estar vacío')
    .max(50, 'Icono debe tener máximo 50 caracteres')
    .regex(
      /^[a-zA-Z0-9\-_]+$/,
      'Icono solo puede contener letras, números, guiones y guiones bajos',
    )
    .refine(
   ...
   */
  static readonly tagsSchema = z
    .array(
      z
        .string()
        .min(1, 'Tag no puede estar vacío')
        .max(
          TASK_CONFIG.MAX_TAG_LENGTH,
          `Tag debe tener máximo ${TASK_CONFIG.MAX_TAG_LENGTH} caracteres`,
        )
        .regex(/^[\w\s\-áéíóúÁÉÍÓÚñÑ]+$/, 'Tag contiene caracteres no válidos'),
    )
    .max(
      TASK_CONFIG.MAX_TAGS_COUNT,
      `Máximo ${TASK_CONFIG.MAX_TAGS_COUNT} tags permitidos`,
    );

  /**
   * Esquema para URLs de attachments
   */
  static readonly attachmentsSchema = z
    .array(z.string().url('URL de attachment inválida'))
    .max(
      TASK_CONFIG.MAX_ATTACHMENTS_COUNT,
      `Máximo ${TASK_CONFIG.MAX_ATTACHMENTS_COUNT} attachments permitidos`,
    );

  /**
   * Esquema para fechas futuras
   */
  static readonly futureDateSchema = z
    .string()
  ...
  static readonly createCategorySchema = z.object({
    name: z
      ...
      .max(
        CATEGORY_CONFIG.MAX_NAME_LENGTH,
        `Nombre debe tener máximo ${CATEGORY_CONFIG.MAX_NAME_LENGTH} caracteres`,
      )
      .regex(
    ...
    description: z
      ...
      .max(
        ...
        `Descripción debe tener máximo ${CATEGORY_CONFIG.MAX_DESCRIPTION_LENGTH} caracteres`,
      )
      .optional(),
    color: ValidationMiddleware.hexColorSchema.default(
      CATEGORY_CONFIG.DEFAULT_COLOR,
    ),
   ...
   */
  static readonly updateCategorySchema =
    ValidationMiddleware.createCategorySchema.partial();
  ...
  static readonly createTaskSchema = z.object({
    title: z
      ...
      .max(
        ...
        `Título debe tener máximo ${TASK_CONFIG.MAX_TITLE_LENGTH} caracteres`,
      ),
    description: z
      .string()
      .max(
        TASK_CONFIG.MAX_DESCRIPTION_LENGTH,
        `Descripción debe tener máximo ${TASK_CONFIG.MAX_DESCRIPTION_LENGTH} caracteres`,
      )
      .optional(),
    dueDate: ValidationMiddleware.futureDateSchema.optional(),
    priority: z
      .nativeEnum(TASK_PRIORITIES, {
        errorMap: () => ({
          message: `Prioridad debe ser: ${Object.values(TASK_PRIORITIES).join(', ')}`,
        }),
      })
    ...
    status: z
      .nativeEnum(TASK_STATUSES, {
        errorMap: () => ({
          message: `Estado debe ser: ${Object.values(TASK_STATUSES).join(', ')}`,
        }),
      })
    ...
    estimatedHours: z
      ...
      .max(
        ...
        `Horas estimadas no puede ser mayor a ${TASK_CONFIG.MAX_ESTIMATED_HOURS}`,
      )
      .optional(),
  });
   ...
   */
  static readonly updateTaskSchema =
    ValidationMiddleware.createTaskSchema.partial();
  ...
  static readonly taskFiltersSchema = z.object({
    ...
    search: z
      ...
      .regex(
        ...
        'Búsqueda contiene caracteres no válidos',
      )
      .optional(),
  });
  ...
  static readonly bulkOperationSchema = z.object({
    ids: z
      .array(ValidationMiddleware.cuidSchema)
      .min(
        VALIDATION_CONFIG.BULK.MIN_ITEMS,
        `Mínimo ${VALIDATION_CONFIG.BULK.MIN_ITEMS} elementos requeridos`,
      )
      .max(
        VALIDATION_CONFIG.BULK.MAX_ITEMS,
        `Máximo ${VALIDATION_CONFIG.BULK.MAX_ITEMS} elementos permitidos`,
      ),
  });
  ...
  static validate<T>(
    ...
    return (req: Request, res: Response, next: NextFunction): void => {
      try {
        ...
        if (!result.success) {
          const formattedErrors = ValidationMiddleware.formatZodErrors(
            result.error,
          );

          ValidationMiddleware.logValidationErrors(
            req,
            formattedErrors,
            target,
          );
          ...
          const statusCode =
            ValidationMiddleware.getStatusCodeForTarget(target);
  ...
  static validateIdParam() {
    return ValidationMiddleware.validate(
      ValidationMiddleware.idParamSchema,
      'params',
    );
  }
  ...
  static validateSort() {
    return ValidationMiddleware.validate(
      ValidationMiddleware.sortSchema,
      'query',
    );
  }
  ...
  static validateTaskFilters() {
    return ValidationMiddleware.validate(
      ValidationMiddleware.taskFiltersSchema,
      'query',
    );
  }
  ...
  static validateBulkOperation() {
    return ValidationMiddleware.validate(
      ValidationMiddleware.bulkOperationSchema,
      'body',
    );
  }
  ...
  static validatePaginationAndSort() {
    const combinedSchema = ValidationMiddleware.paginationSchema.merge(
      ValidationMiddleware.sortSchema,
    );
  ...
  private static formatZodErrors(zodError: ZodError): ValidationErrorDetail[] {
    return zodError.errors.map((error) => ({
      ...
      receivedValue:
        error.code === 'invalid_type' ? undefined : (error as any).received,
      code: error.code,
    }));
  }
   ...
   */
  private static getStatusCodeForTarget(
   ...
   */
  private static getMessageForTarget(
    target: 'body' | 'query' | 'params',
  ): string {
    switch (target) {
      case 'params':
  ...
  private static logValidationErrors(
    ...
    const logContext = {
      ...
      requestBody:
        target === 'body'
          ? ValidationMiddleware.sanitizeBodyForLogging(req.body)
          : undefined,
  ...
  static isFutureDate(
    dateString: string,
    minOffsetMinutes: number = TASK_CONFIG.MIN_DUE_DATE_OFFSET_MINUTES,
  ): boolean {
    try {
