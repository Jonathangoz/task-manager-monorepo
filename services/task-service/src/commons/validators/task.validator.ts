  ...
  .refine(
    (date) => {
      ...
      const minDate = new Date(
        now.getTime() + TASK_CONFIG.MIN_DUE_DATE_OFFSET_MINUTES * 60000,
      );
...
const titleSchema = z
  ...
  .max(
    ...
    `Title must be at most ${TASK_CONFIG.MAX_TITLE_LENGTH} characters`,
  )
...
const descriptionSchema = z
  ...
  .max(
    ...
    `Description cannot exceed ${TASK_CONFIG.MAX_DESCRIPTION_LENGTH} characters`,
  )
  .optional();
...
const prioritySchema = z.enum(
  Object.values(TASK_PRIORITIES) as [string, ...string[]],
  {
    errorMap: () => ({
...
const optionalEstimatedHoursSchema = z
  .union([estimatedHoursSchema, z.null()])
  .optional();
...
const optionalActualHoursSchema = z
  .union([actualHoursSchema, z.null()])
  .optional();
...
const sortingSchema = z.object({
  sortBy: z
    .enum(Object.values(SORT_FIELDS) as [string, ...string[]], {
      errorMap: () => ({
        message: `sortBy must be one of: ${Object.values(SORT_FIELDS).join(', ')}`,
      }),
    })
    .optional(),
  sortOrder: z
...
const baseQuerySchema = z.object({
  ...
  priority: z
    .union([prioritySchema, z.array(prioritySchema)])
    .optional()
    .transform((val) => (Array.isArray(val) ? val : val ? [val] : undefined)),
...
export type BulkUpdateTasksInput = z.infer<
