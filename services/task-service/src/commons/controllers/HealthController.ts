...
export class HealthController {
  ...
  public async readinessCheck(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res
        .status(isReady ? HTTP_STATUS.OK : HTTP_STATUS.SERVICE_UNAVAILABLE)
        .json(response);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
  ...
  public async detailedHealthCheck(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (isHealthy) {
        healthCheck.passed('detailed', responseTime, {
          checks,
          metrics: systemMetrics,
        });
      } else {
  ...
  public async databaseHealthCheck(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (result.status) {
        ...
        healthCheck.failed(
          'database',
          new Error(result.error || 'Database check failed'),
          result.responseTime,
        );
      }
      ...
      res
        .status(
          result.status ? HTTP_STATUS.OK : HTTP_STATUS.SERVICE_UNAVAILABLE,
        )
        .json(response);
    } catch (error) {
      const errorMessage =
  ...
  public async redisHealthCheck(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (result.status) {
        ...
        healthCheck.failed(
          'redis',
          new Error(result.error || 'Redis check failed'),
          result.responseTime,
        );
      }
  ...
  public async authServiceHealthCheck(
    ...
    try {
      ...
      if (result.status) {
        ...
        healthCheck.failed(
          'auth-service',
          new Error(result.error || 'Auth service check failed'),
          result.responseTime,
        );
      }
