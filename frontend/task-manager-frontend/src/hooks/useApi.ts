...
import { HTTP_STATUS, ERROR_CODES, REQUEST_HEADERS } from '@/lib/constants';
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          if (attempts < maxAttempts) {
            console.log(
              `API Retry [${requestId}] - Waiting ${mergedOptions.retryDelay}ms before attempt ${attempts + 1}`,
            );
...
export const useApiParallel = <T = any>(
  ...
  const executeParallel = useCallback(
    async (configs: RequestConfig[]): Promise<(T | null)[]> => {
      setState({
        isLoading: true,
        errors: [],
        results: [],
